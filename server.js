  ...
  try {
    ...
    await pool.query(
      `INSERT INTO documents (fiscal_year_id, month_key, doc_type, filename, mime_type, file_data, metadata)
       VALUES ($1,$2,'playground_staff_hours',$3,$4,$5,'{}')`,
    ...
    for (const cols of dataRows) {
      ...
      if (!staff) { if (!unmatched.includes(fullName)) unmatched.push(fullName); continue; }
    ...
    res.json({ ok: true, imported, unmatched, total_rows: dataRows.length });
